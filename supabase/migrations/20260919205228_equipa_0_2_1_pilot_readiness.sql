...
-- audit_events_occurred_idx ja cobre (occurred_at desc); nao criar indice duplicado.
...
begin
 ...
 with activity as (
   select date_trunc('day',w.withdrawn_at)::date as activity_date,count(*)::bigint n
   ...
   group by 1
   union all
   select date_trunc('day',r.start_at)::date as activity_date,count(distinct coalesce(r.batch_id::text||':'||r.start_at::text,r.id::text))::bigint n
   ...
   where r.start_at>=v_chart_from and r.start_at<p_to
    ...
    and (p_group is null or e.school_group=p_group)
   group by 1
 ), grouped as (select activity_date,sum(n)::bigint total from activity group by activity_date)
 select coalesce(jsonb_agg(jsonb_build_object('day',activity_date,'total',total) order by activity_date),'[]'::jsonb) into v_days from grouped;
