...
function pageTitle() {
  return ({ dashboard: "Visão geral", equipment: "Equipamentos", withdrawals: "Retiradas", history: "Histórico", carts: "Carrinhos", maintenance: "Manutenção", admin: "Administração", audit: "Auditoria" })[state.view] || "Equipa";
}
...
async function renderWithdrawals(){
  ...
  shell(`<section class="operations-page" aria-label="Situação atual dos equipamentos">
    <div class="ops-breadcrumb">Início › Retiradas</div>
    <header class="ops-heading"><div class="ops-heading-icon">${uiIcon("withdrawals",24)}</div><div><h1>Retiradas</h1><p>Identifique os equipamentos disponíveis, em uso e os que precisam de atenção.</p></div><button class="button primary ops-create" id="withdrawal-new-desktop" type="button">${uiIcon("plus",18)} Nova retirada</button></header>
...
async function renderReports() {
  ...
  shell(`<section class="panel workspace-panel reports-workspace"><div class="panel-head workspace-head report-top-head"><div class="report-hero"><span class="report-hero-icon">${uiIcon("reports",24)}</span><div><span class="eyebrow">Relatórios</span><h2>Relatórios</h2><p>Acompanhe uso, manutenções e retiradas dos equipamentos da escola.</p></div></div><div class="report-head-actions"><button class="button ghost" id="report-export" type="button">${uiIcon("upload",18)}<span>Exportar relatório</span></button></div></div>
